---
title: 'Data Science Term Project: Forecasting political trouble'
author: "Zsopi"
date: "6 March 2016"
fontsize: 12pt
...
* __polity2__: Political system variable. Denotes how authoritarian or democratic is in the country in the given year. Source: Polity IV:  Regime Authority Characteristics and Transitions Datasets from the [**Center for Systemic Peace**](http://www.systemicpeace.org/inscrdata.html).
...
###Results show that the 17-24 age bracket is the best definition of "young" as far as predicting political trouble is concerned, so I will use this definition, but the results are close to each other, so the age range does not make much difference. I will re-generate demographic variables in the following chunk
...
##Some exploratory data, demographics
...
##I did not manage to make ensemble work (some isses with Rcurl), so I will just use the average probability forecast of the final models as my final probabilities. Probabilities are fine if we think of this as an input for decisionmakers etc.
...
##Let's see how 2016 looks like in terms of political trouble predicitons. Worst countries are at the top. This is the final test of the prediciton, but can be evaluated only next year :) However, the results look pausible

```{r}
View(data_w_fcast%>%filter(year==2016)%>%mutate(avg_predict=(gbm_predict+rf_predict+dl_predict)/3)%>%arrange(desc(avg_predict)))

pred_2016<-data_w_fcast%>%filter(year==2016)%>%mutate(avg_predict=(gbm_predict+rf_predict+dl_predict)/3)%>%arrange(desc(avg_predict))

pred_2016

pred_2016_nrw<-pred_2016%>%select(year,country,iso3c,gbm_predict,rf_predict,dl_predict,avg_predict)

pred_2016_nrw

```

Another way of showing 2016 forecasts

```{r comment=''}

#cat(readLines(pred_2016_nrw), sep = '\n')

```

##Discussion

###* Clearly, h2o makes forecasts even if some variables are missing - I presume it inputs averages, but I will have to check that

###* Some of the forecasts with incomplete data look quite good, that is why I left these in (PSE is for Palestinian Authority for example, SOM is for Somalia)

###* There is a risk that there was some covert overfitting, despite the fact that cross validation and early stopping was used: this is partly time series data, and countries are similar from one year to another (year variable was not included in the estimates of course)

###* I was surprised by the fact how different Deep Learning results were from the other two methods (also a lot lower AUC). I could not figure out why that is the case - overfitting in the case of the other two methods could be an explanation, but this is just a guess
...
### * The best thing in this work was that it gathered a dataset and put it in a usable format - this will make further estimations possible 

### * This is a work in progress, and despite the overtime, I could not make it as nice as I would have liked

### * But it was fun and I learned a lot :)

### * Also, some of the results are interesting, as dicussed above, and I would like to investigate them further
