...
Deep Learning (Neural network) has a significantly lower AUC than the other 2 models 
```{r}
ml.dl_final
```

##I did not manage to make ensemble function in h2o work (some isses with Rcurl), so I will just use the average probability forecast of the final models as my final probabilities. Probabilities are fine if we think of this as an input for decisionmakers etc., and it can be converted to a 0/1 forecast easily if needed
...
##Let's see how 2016 looks like in terms of political trouble predicitons. Worst countries are at the top. This is the final test of the prediciton, but can be evaluated only next year :) However, the results look broadly pausible...
...
###* Some of the forecasts with incomplete data look quite good, that is why I left these in (PSE is for Palestinian Authority for example, SOM is for Somalia, both have only partial data - and countrynames() function does not even know them - , but political instability is plausible)
...
###* Interestingly, the models predict an increase in the probability of political trouble for poor countries that increase their democratic scores(polity2)
...
### * The best thing in this work was that it gathered a dataset and put it in a usable format - this will make further estimations and discovery possible 
